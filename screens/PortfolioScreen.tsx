...
export default function PortfolioScreen({ navigation }: any) {
  ...
  const { balance: usdcBalance, refetch: refetchUSDC } =
    useUSDCBalance(typedAddress);
  ...
  const getPositionForVault = (type: VaultType) => {
    ...
    if (type === VaultType.Stable)
      return {
        shares: fromBigInt(position.stableShares),
        value: fromBigInt(position.stableAssets),
      };
    if (type === VaultType.Growth)
      return {
        shares: fromBigInt(position.growthShares),
        value: fromBigInt(position.growthAssets),
      };
    if (type === VaultType.Turbo)
      return {
        shares: fromBigInt(position.turboShares),
        value: fromBigInt(position.turboAssets),
      };
    ...
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-zinc-950">
      ...
      >
        ...
        <View className="mb-8">
          ...
          <Text className="text-4xl font-bold text-gray-900 dark:text-zinc-50 mt-1">
          ...
          <View className="flex-row items-center mt-2">
            ...
            <Text className="text-gray-500 dark:text-zinc-500 text-xs ml-2">
              past 24h
            </Text>
          </View>
        </View>
        ...
        <View className="mb-8">
          ...
          {activePositions.length === 0 ? (
            <View className="bg-white dark:bg-zinc-900/50 rounded-xl p-8 items-center border border-gray-200 dark:border-zinc-800 border-dashed">
              <Text className="text-gray-500 dark:text-zinc-400 text-center mb-4">
            ...
            <View className="gap-4">
              {activePositions.map((pos) => {
                ...
                return (
                  <View
                    key={pos.id}
                    className="bg-white dark:bg-zinc-900/50 rounded-xl p-4 border border-gray-200 dark:border-zinc-800 shadow-sm"
                  >
                    <View className="flex-row justify-between items-start mb-3">
                      <View>
                        <Text className="text-base font-bold text-gray-900 dark:text-zinc-50">
                          {pos.name}
                        </Text>
                      ...
                      <View className="items-end">
                        <Text className="text-gray-900 dark:text-zinc-50 font-semibold text-base">
                    ...
                    <View className="flex-row gap-2">
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          navigation.navigate("deposit", { vaultId: pos.id })
                        }
                      >
                        <Text className="text-white dark:text-black text-center font-semibold text-sm">
                          Deposit
                        </Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          navigation.navigate("withdraw", { vaultId: pos.id })
                        }
                      >
                        <Text className="text-gray-900 dark:text-zinc-200 text-center font-semibold text-sm">
                          Withdraw
                        </Text>
                      </TouchableOpacity>
                    </View>
        ...
        <View className="mb-8">
          ...
          <View className="bg-white dark:bg-zinc-900/50 rounded-xl border border-gray-200 dark:border-zinc-800 overflow-hidden shadow-sm">
            {(historyLoading || refreshing) && !history.length && (
              <ActivityIndicator className="py-8" color="#FDC700" />
            )}
            ...
            {history.slice(0, 10).map((item, idx) => {
              ...
              const isIncoming =
                isDeposit || (isUSDCTransfer && item.direction === "in");
              ...
              if (item.vaultType !== undefined) {
                subTitle =
                  VaultTypeName[item.vaultType as VaultType] || "Unknown Vault";
              }
              ...
              if (isDeposit) {
                ...
                amountDisplay = `${formatShares(
                  fromBigInt(item.shares)
                )} Shares`;
              ...
              return (
                <TouchableOpacity
                  ...
                  className={`p-4 flex-row justify-between items-center ${
                    idx !== (history.length > 10 ? 9 : history.length - 1)
                ...
                >
                  <View className="flex-row items-center gap-3">
                    ...
                    <View>
                      ...
                      <Text className="text-[10px] text-gray-500 dark:text-zinc-400">
                        {subTitle || "Vault interaction"}
                      </Text>
                    </View>
                  </View>
                  <View className="items-end">
